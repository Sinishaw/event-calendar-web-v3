...
import { CategoryOption, TopicOption } from '@/services/remote-config.service';

interface TopicContentFormProps {
  content?: TopicContent | null; // If editing
  categories: CategoryOption[];
  topics: TopicOption[];
}

export default function TopicContentForm({ content, categories, topics }: TopicContentFormProps) {
  ...
  // Form Fields State
  const [topic, setTopic] = useState(content?.topic ?? (topics[0]?.value || ''));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Validations
    if (!topic) {
      setError('Topic channel selection is required');
      return;
    }
    if (!companyName) {
    ...
    try {
      ...
      // If editing, URL must contain the ORIGINAL topic parameter and original id
      const url = isEdit ? `/api/content` /* Wait, let's call correct endpoint */ : '/api/topics';
      const requestUrl = isEdit ? `/api/topics/${content?.topic}/${content?.id}` : '/api/topics';
      ...
      if (!res.ok) {
        throw new Error(result.error || 'Failed to save topic content article');
      }

      setSuccess(isEdit ? 'Topic article updated successfully!' : 'Topic article created successfully!');

      if (!isEdit) {
        router.push(`/dashboard/topics/${result.data.topic}/${result.data.id}`);
        router.refresh();
      } else {
        router.push(`/dashboard/topics/${result.data.topic}/${result.data.id}`);
        router.refresh();
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
          ...
          <div>
            <div className="form-row">
              <div className="form-group">
                <label htmlFor="topic">Topic Channel</label>
                <select
                  id="topic"
                  value={topic}
          ...
          <div style={{ borderLeft: '1px solid var(--border)', paddingLeft: '2rem' }}>
            ...
            <div className="form-group">
              <label htmlFor="logoUrl">Custom Icon URL (Optional)</label>
              <input
                id="logoUrl"
                type="url"
                value={logoUrl}
        ...
        <div className="flex gap-3" style={{ justifyContent: 'flex-start' }}>
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? 'Saving…' : isEdit ? 'Submit Changes' : 'Save Topic Content'}
          </button>
          <button
            type="button"
            className="btn btn-secondary"
            onClick={() => router.push(isEdit ? `/dashboard/topics/${content?.topic}/${content?.id}` : '/dashboard/topics')}
            disabled={loading}
          >
            Cancel
          </button>
