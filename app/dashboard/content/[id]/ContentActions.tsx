...
export default function ContentActions({ content, isPublisher }: ContentActionsProps) {
  ...
  async function handleApprove() {
    ...
    try {
      ...
      setSuccess('Content approved and published successfully!');
      router.refresh();
    } catch (err: any) {
  ...
  async function handleUnpublish() {
    ...
    try {
      const res = await fetch(`/api/content/${content.id}/unpublish`, {
        method: 'POST',
      });
      ...
      setSuccess('Content unpublished and reverted to Draft successfully!');
      router.refresh();
    } catch (err: any) {
      setError(err.message || 'Something went wrong');
    } finally {
  ...
  async function handleNotify() {
    ...
    try {
      const res = await fetch(`/api/content/${content.id}/notify`, {
        method: 'POST',
      });
      ...
      setSuccess('FCM Push notification broadcasted successfully!');
      router.refresh();
    } catch (err: any) {
      setError(err.message || 'Something went wrong');
    } finally {
  ...
  async function handleDelete() {
    if (!window.confirm(`Are you sure you want to delete article "${content.title}"?`)) {
      return;
    }
    ...
    try {
      ...
      setSuccess('Content archived successfully!');
