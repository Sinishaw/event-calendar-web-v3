...
import { approveTermRecord } from '@/services/terms.service';
...
export async function POST(req: NextRequest, props: Params) {
  ...
  try {
    await approveTermRecord(companyId, id, session.email as string);
    return NextResponse.json({ success: true });
  } catch (err: any) {
    console.error(`POST /api/terms/${id}/approve error:`, err);
    return NextResponse.json({ error: err.message || 'Failed to approve terms document' }, { status: 500 });
  }
}
