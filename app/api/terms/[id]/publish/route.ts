...
import { publishTermRecord } from '@/services/terms.service';
...
export async function POST(req: NextRequest, props: Params) {
  ...
  try {
    const result = await publishTermRecord(companyId, id, session.email as string, override);
    if (!result.ok) {
      if (result.conflict) {
        return NextResponse.json(
          {
            error: 'Conflict: Another terms document is currently active.',
    ...
    console.error(`POST /api/terms/${id}/publish error:`, err);
    return NextResponse.json({ error: err.message || 'Failed to publish terms document' }, { status: 500 });
  }
}
