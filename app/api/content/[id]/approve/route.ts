...
import { approveContent } from '@/services/content.service';
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    await approveContent(companyId, id, session.email);
    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error(`API POST approve content ${id} error:`, error);
